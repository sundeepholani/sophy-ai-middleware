  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <Link href={projectPath(projectId, 'evals')} className="text-sm text-primary hover:underline">
            ← Evals
          </Link>
        ...
        {run.status === 'running' && <EvalRunCancel projectId={projectId} runId={run.id} />}
      </div>
      ...
      {run.samples.length > 0 && <SamplesTable projectId={projectId} run={run} />}
    </div>
  );
}
...
function SamplesTable({ projectId, run }: { projectId: string; run: EvalRunDetail }) {
  return (
    <div className="space-y-1.5">
      ...
      <div className="overflow-x-auto rounded-lg border bg-card">
        <Table>
          ...
          <TableBody>
            {run.samples.map((s) => (
              <SampleRow key={s.id} projectId={projectId} sample={s} run={run} />
            ))}
          </TableBody>
        </Table>
      </div>
  ...
  return (
    <TableRow>
      ...
      <TableCell className="text-right">
        ...
        {s.status !== 'pending' && (
          <Button
            render={<Link href={projectPath(projectId, `logs/${s.id}`)} />}
            nativeButton={false}
            size="sm"
            variant="ghost"
          >
